...
## Método de respaldo (Fallback) para evitar que el juego quede congelado si la escena de la demo falla
func _fallback_to_main_menu() -> void:
	if has_node("/root/SceneManager") or is_instance_valid(SceneManager):
		SceneManager.goto_main_menu()
	else:
		# Si ni el SceneManager está disponible, forzamos un cierre o volvemos a una escena raíz básica
		push_error("[%s] SceneManager no disponible para el fallback." % ES_NAME_CLASS)
...
func _on_return_pressed() -> void:
	EssenceLogger.system_info("[%s/_on_return] Procesando salida del panel de guardado..." % ES_NAME_CLASS)
	...
	# Llamamos al método genérico del Addon. El framework hará la mezcla en segundo plano
	# sin enterarse jamás de qué significan tus variables de historia o vestuario.
	SaveManager.patch_cache_and_prepare_load(cambios_juego)
	
	# Cambiamos de escena usando tus constantes centralizadas
	var target_scene: String = DemoItemsRoute.TESTMAINGAME_SCENE
	if target_scene != "" and ResourceLoader.exists(target_scene):
		get_tree().change_scene_to_file(target_scene)
	else:
		push_error("[%s] Error: No se encontró la escena principal." % ES_NAME_CLASS)
