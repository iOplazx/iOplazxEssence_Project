...
func _ir_a_advertencia():
	print("6. Iniciando módulo de Advertencia...")
	
	# Instanciamos el nuevo componente modular
	var pantalla_advertencia = EssenceWarningScreen.new()
	add_child(pantalla_advertencia)
	
	# Conectamos su señal de finalización a nuestro siguiente paso (la carga)
	pantalla_advertencia.warning_completed.connect(_ir_a_carga)
	
	# Le pasamos la configuración y dejamos que trabaje solo
